...
网络结构: 多层卷积结构 判别器在卷积层后的全连接层concat One_Hot条件  而 生成器在开头concat One_Hot 条件
...
class Discriminator(tf.keras.Model):
    def __init__(self,in_shape,label_dim):
        ...
        2+2+2+2+2=10 一共十个参数个数(指独立大参数self.w self.b的个数)
        """
    ...
    def call(self,x,y,training=True):
        ...
        # l3_out = tf.nn.sigmoid(dense_l3) 要和标签统一维度 不能再是sigmoid 而应该是tanh
