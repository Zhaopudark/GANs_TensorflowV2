...
def loadMinistLable(filename):
    ...
    if head[0]!=2049:
        ...
        return labels
def load_data(get_new=True,normalization=False,one_hot=False,detype=np.float32):
    if get_new==True:
        train_images = loadMinistImage(mnist_path+"train-images.idx3-ubyte")
        train_labels = loadMinistLable(mnist_path+"train-labels.idx1-ubyte")
        test_images = loadMinistImage(mnist_path+"t10k-images.idx3-ubyte")
        test_labels = loadMinistLable(mnist_path+"t10k-labels.idx1-ubyte")
        np.savez(mnist_path+"mnist.npz",k1=train_images,k2=train_labels,k3=test_images,k4=test_labels)
    else:
        npzfile=np.load(mnist_path+'mnist.npz') 
