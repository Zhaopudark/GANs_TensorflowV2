...
"""
基于tensorflow 高阶API
SNGAN  Spectra Normalization GAN 谱范数正则化应用于GAN
损失函数: 还是基于WGAN理论 W 距离loss  考察真假样本的分布差异 判别器需要满足Lipschitz-1 Lipschitz-K 约束 所以不能加sigmoid约束范围
        发现只要约束权值矩阵的谱范数 就可以约束权值 使得权值矩阵形成的函数满足Lipschitz-1约束
        对每一层权重矩阵除以其谱范数即可
        谱范数采用幂迭代逼近
        若是卷积层 需要对卷积层的W做变换 变为等价的权重矩阵才行 不需要处理偏置
...
初始化: xavier初始化  即考虑输入输出维度的 glorot normal
训练： 判别器5次 生成器1次
"""
import my_mnist  
    ...
    def sn_op(self):
        for w_l in self.trainable_variables:
            if w_l.shape[0]>1:
                ...
                #u = tf.matmul(v0,w_l)#但是不用算两次 可以这么操作
                for iter_k in range(3):
                    ...
                    v = v/tf.norm(v,ord='euclidean')
                u = tf.matmul(v,w_l)
                u = u/tf.norm(u,ord='euclidean')#vk先得到 uk再多做一次才得到
